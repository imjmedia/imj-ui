...
const SubSubtitle = (props: SubtitlesProps) => {
	const { subsubtitle, modo, style } = props;
	return (
		<div style={style}>
			<h3
				style={{
	...
	return (
		<>
			...
			>
				...
				<div style={{ width: '100%' }}>
					<BodyText text={property} style={{ textAlign: 'center' }} modo={modo} />
				</div>
			</div>
		</>
	);
...
export const TitlesCards = (props: TitlesCardsProps) => {
	const { clave, type, state, property, color, modo, subclave } = props;
	return (
		<div
			style={{
				display: 'flex',
		...
		>
			<div>
				...
				{subclave ? (
					<SubSubtitle subsubtitle={subclave} modo={modo} style={{ opacity: '.75' }} />
				) : (
					<></>
				)}
				<Subtitle subtitle={type} modo={modo} />
			</div>
			{property ? <Property property={property} estado={state} modo={modo} /> : <></>}
		</div>
	);
};
